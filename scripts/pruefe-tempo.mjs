...
async function live(basis) {
  ...
  for (const [name, weg] of SEITEN) {
    ...
    console.log(`  ${name.padEnd(14)} LCP ${String(m.lcp).padStart(5)} ms | CLS ${String(m.cls).padStart(6)}`
      + ` | Bild p95 ${String(m.p95).padStart(5)} ms | ${String(kb).padStart(5)} KB`
      + ` (JS ${Math.round(js / 1024)}) | Fehler ${fehler}`);
    if (m.lcp > BUDGET.lcp) maengel.push(`${name}: LCP ${m.lcp} ms (Budget ${BUDGET.lcp})`);
